...
# then log output of file /code/bin/cosmotiand
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/cosmotiand \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/cosmotiand | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/cosmotiand /usr/bin/cosmotiand
...
CMD ["/usr/bin/cosmotiand", "version"]
