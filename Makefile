...
HTTPS_GIT := github.com/unroar12/cosmotian.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=cosmotian \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=cosmotiand \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/cosmotiand ./cmd/cosmotiand
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/cosmotiand.exe ./cmd/cosmotiand
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/cosmotiand
...
draw-deps:
	...
	@goviz -i ./cmd/cosmotiand -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c cosmotian --local -f chains.yaml
endif
...
set-testnet-configs:
	cosmotiand config set client chain-id chainid-1
	cosmotiand config set client keyring-backend test
	cosmotiand config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | cosmotiand keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | cosmotiand keys add acc1 --recover`
